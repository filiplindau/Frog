...
class FrogCalculation(object):
    ...
    def applyIntensityData(self, I_w_tau=None):
        ...
        krn = self.progs.progs['applyIntensityData'].apply_intensity_data
        krn.set_scalar_arg_dtypes((None, None, np.int32))
    ...
    def minZerrKernSD_gpu(self):
        ...
        root.debug(''.join(('Poly: ', str(X6), ' x^6 + ', str(X5), ' x^5 + ', str(X4), ' x^4 + ', str(X3), ' x^3 + ',
    ...
    def updateEt_gp(self, algo='SHG'):
        ...
        if algo == 'SHG':
            ...
            root.debug(''.join(('Poly: ', str(p1[4]), ' x^4 + ', str(p1[3]), ' x^3 + ', str(p1[2]), ' x^2 + ',
                                str(p1[1]), ' x + ', str(p1[0]))))
        elif algo == 'SD' or algo == 'TG':
            # Calculate the gradient of the functional distance:
            if self.useCL == True:
                self.gradZSD_gpu()
            else:
...
if __name__ == '__main__':
    ...
    frog = FrogCalculation(useCPU=False)

    #    frog.initPulseFieldPerfect(128, dt, 800e-9)
    #    frog.initPulseFieldGaussian(N, dt, l0, 50e-15)

    frog.init_pulsefield_random(N, 13e-15, l0 / 2)
    #     frog.conditionFrogTrace(IfrogSD[::-1,:], l[0], l[-1], t[0], t[-1])
    #     frog.loadFrogTrace('./frogtrace_2016-03-31_11h29_image.png', thr=0.15, lStartPixel=0, lStopPixel=700, tStartPixel=0, tStopPixel=-1)
