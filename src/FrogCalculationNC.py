...
class FrogCalculation(object):
    ...
    def init_pulsefield_random(self, N, t_res, l0, seed=0):
        """ Initiate signal field with parameters:
        t_res: time resolution of the reconstruction
        N: number of points in time and wavelength axes
        l0: center wavelength
    ...
    def generateEsig_t_tau_SD(self):
        ...
        t0 = time.clock()
        krn = self.progs.progs['generateEsig_t_tau_SD'].generate_Esig_t_tau_sd
    ...
    def load_frog_trace(self, filename, thr=0.0, lStartPixel=0, lStopPixel=-1, tStartPixel=0, tStopPixel=-1):
        ...
        picF = self.filter_frog_trace(picN, 3, thr)

        self.condition_frog_trace(picF[tStartPixel:tStopPixel, lStartPixel:lStopPixel],
                                  lData[lStartPixel], lData[lStopPixel], tData[tStartPixel], tData[tStopPixel], thr)

    def filter_frog_trace(self, Idata, kernel=5, thr=0.1):
        Idata_f = medfilt2d(Idata, kernel) - thr
        Idata_f[Idata_f < 0.0] = 0.0
        return Idata_f

    def condition_frog_trace(self, Idata, l_start, l_stop, tau_start, tau_stop, thr=0.15):
    ...
    def calculateGradZSHG(self):
        ...
        dZre = (-np.conj(self.Esig_t_tau_p.flatten()) * Et_mat[self.shiftIndsNeg] +
                ...
                np.conj(Et_mat) * Et_mat[self.shiftIndsPos] * np.conj(Et_mat[self.shiftIndsPos])).reshape(N, N).sum(
            axis=0)
    ...
    def calculateGradZSHG_t(self):
        ...
        Et_tau_p_c = np.fliplr(
            np.triu(np.tril(np.fliplr(np.conj(Et_mat.flatten()[self.shiftIndsNeg]).reshape(N, N)), N / 2 - 1), -N / 2))
        Et_tau_n_c = np.triu(np.tril(np.conj(Et_mat.flatten()[self.shiftIndsPos]).reshape(N, N), N / 2 - 1), -N / 2)
        Esig_p = np.fliplr(
            np.triu(np.tril(np.fliplr(np.conj(Esig.flatten()[self.shiftIndsNeg]).reshape(N, N)), N / 2 - 1), -N / 2))
        self.dZ = np.real((Et_mat * np.conj(Et_tau_n_c) * Et_tau_n_c -
                           Esig * Et_tau_n_c +
                           Et_mat * np.conj(Et_tau_p_c) * Et_tau_p_c -
    ...
    def dZdE_test(self, Ein, Et):
        ...
        Esig = Ein
        Et_tau_p_c = np.fliplr(
            np.triu(np.tril(np.fliplr(np.conj(Et_mat.flatten()[shiftIndsNeg]).reshape(N, N)), N / 2 - 1), -N / 2))
        Et_tau_n_c = np.triu(np.tril(np.conj(Et_mat.flatten()[shiftIndsPos]).reshape(N, N), N / 2 - 1), -N / 2)
        Esig_p = np.fliplr(
            np.triu(np.tril(np.fliplr(np.conj(Esig.flatten()[shiftIndsNeg]).reshape(N, N)), N / 2 - 1), -N / 2))
    ...
    def runCycleSVD(self, cycles=1, algo='SHG'):
    ...
    def runCycleGP(self, cycles=1, algo='SHG'):
...
if __name__ == '__main__':
    ...
    dt = 13e-15
    frogNC.init_pulsefield_random(N, dt, l0)
    #     frogNC.conditionFrogTrace(Ifrog, l[0], l[-1], t[0], t[-1])
    #     frogNC.loadFrogTrace('./frogtrace_2016-03-31_11h29_image.png', thr=0.2, lStartPixel=50, lStopPixel=665, tStartPixel=18, tStopPixel=65)
    frogNC.load_frog_trace('./frogtrace_2017-02-16_14h44_image.png', thr=0.25)
