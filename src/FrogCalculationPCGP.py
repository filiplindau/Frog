...
class FrogCalculation(object):
    ...
    def condition_frog_trace(self, Idata, l_start, l_stop, tau_start, tau_stop, n_frog=256, thr=0.15, filter_img=True):
        ...
        root.info(''.join(('Time spent: ', str(time.time() - t0))))
    ...
    def generate_Esig_t_tau(self, algo='SHG'):
        ...
        root.debug(''.join(('Time spent: ', str(time.time() - t0))))
    ...
    def run_cycle_pc(self, frog_type="PG"):
        pass
...
if __name__ == '__main__':
    ...
    frog.init_pulsefield_random(N, dt, l0+5e-9)
